...
class EndoArssRuntime:
    ...
    def _render_segmentation_overlay_png_base64(
        self, image_rgb: Image.Image, seg_mask: torch.Tensor
    ) -> str:
        # Render semantic segmentation on top of the resized input frame.
        # Output is a base64-encoded PNG so the frontend can display it directly.
        resized = image_rgb.resize(
            (self.input_width, self.input_height),
            resample=Image.Resampling.BILINEAR,
        ).convert("RGBA")

        seg_np = seg_mask.detach().cpu().numpy().astype("int64")

        # DeepLabHead uses 7 segmentation channels (0..6).
        # 0 is treated as background (transparent), others are colored with alpha.
        palette = np.array(
            [
                [0, 0, 0, 0],  # background
                [255, 60, 60, 120],  # red
                [60, 200, 90, 120],  # green
                [60, 130, 255, 120],  # blue
                [255, 210, 70, 120],  # yellow
                [215, 90, 255, 120],  # magenta
                [60, 220, 220, 120],  # cyan
            ],
            dtype=np.uint8,
        )
        ...
        composite = Image.alpha_composite(resized, overlay)
    ...
    def predict(
        self, image_rgb: Image.Image, return_segmentation_overlay: bool = False
    ) -> dict:
        self.load()
        ...
        if return_segmentation_overlay:
            segmentation_overlay_png_base64 = self._render_segmentation_overlay_png_base64(
                image_rgb=image_rgb, seg_mask=seg_mask
            )
        ...
        return {
            ...
            "segmentation_overlay_png_base64": segmentation_overlay_png_base64,
        }
