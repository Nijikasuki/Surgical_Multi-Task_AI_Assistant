...
def analyze_frame(
    ...
    model_output = get_endoarss_runtime().predict(
        image_rgb,
        return_segmentation_overlay=(task_id == "region-and-tool-segmentation"),
    )
    profile = _build_scene_profile(
        task_id=task_id,
        procedure=procedure,
...
def _build_scene_profile(
    ...
    return {
        ...
        "segmentation_overlay_png_base64": (
            model_output.get("segmentation_overlay_png_base64")
            if (task_id == "region-and-tool-segmentation" and model_output is not None)
            else None
        ),
    }
